...
class ModelParser {
  ...
  List<ValidatedElement> _getValidatedElements(List<FieldElement> fields) {
    ...
    for (var field in fields) {
      ...
      if (hasValidatorAnnotations) {
        ...
        final annotatedField = ValidatedElement(
          name: field.name,
          element: field.type.element as InterfaceElement,
          elementType: ElementType.FIELD,
          modelClass: model.element as InterfaceElement,
  ...
  List<Expression> _buildPositionalArguments(
    ...
    if (annotation.isContainerAnnotation) {
      ...
      final str = refer(
          '${containerValidator.getDisplayString(withNullability: false)}()..validationContext = validationContext,');
  ...
  InterfaceElement _getValidatorForModel(InterfaceElement validatedModel) {
    ...
    for (var annotatedElement in library.annotatedWith(genValidatorType)) {
      final model = _getModelFromFirstGenericTypeArgument(
          annotatedElement.element as InterfaceElement);

      if (model.getDisplayString(withNullability: false) == validatedModel.name) {
        found.add(annotatedElement.element as InterfaceElement);
      }
    }
  ...
  DartType _getModelFromFirstGenericTypeArgument(InterfaceElement clazz) {
    final interface =
